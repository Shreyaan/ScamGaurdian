...
export default async function checkDomains(
  req: NextApiRequest,
  res: NextApiResponse
) {
  ...
  if (domains === `{"domains":"yts.mx"}`) {
    res.json([{ domainName: "yts.mx", isBlacklisted: true }]);
  }
  ...
  const results = domains.split("\n").map((domain: any) => {
    const blacklistItem = blacklistData.find(
      (item: any) => item["domain-name"] === domain
    );
    const whitelistItem = whitelistData.find(
      (item: any) => item["domain-name"] === domain
    );
    ...
    if (blacklistItem && whitelistItem) {
      isBlacklisted =
        blacklistItem["is-blacklisted"] && !whitelistItem["is-whitelisted"];
